    ...
    <xsl:template match="/">
        ...
        -->        
              ...
              <xsl:text> CE Dash Count: </xsl:text><xsl:value-of select="$dashCountCE"/></xsl:comment> 
                  ...
                  <xsl:if test="(count(//rdg[@wit[contains(., '#am')]])) &gt; 0"><line x1="{($poemNumber*$x-interval)-35}" x2="{($poemNumber*$x-interval)-35}" y1="0" y2="-{$am*$y-interval}" stroke="#D9007E" stroke-width="3"/><circle cx="{($poemNumber*$x-interval)-35}" cy="-{$am*$y-interval}" r="3" fill="#D9007E" /></xsl:if>
